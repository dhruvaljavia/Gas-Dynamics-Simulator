...
nPart_A = 20; % no. of particles
size_A = 5; % particle diameter
speed_A = 0.5; % initial speed
...
g = 0.0005; % acceleration due to gravity (delta_V/timeStep)
heatIn = 2; % in units of energy/time
criticalEnergy = 0; % Minimum total K.E. of two atoms along line-of-centre for reaction to happen
box_xmax = 100; % max box width
box_ymax = 100; % max box breadth
timeTotal = 1000;
timeStep = 1; % dicretized time
...
for i = 2:itr
    ...
    % detect inter-particle and change velocities + detect inside wall collision
        ...
        for n = m+1:(nPart_A+nPart_B)
            ...
            if (isCollisionPart == 1) || (isCollisionPart == 0 && reactiveGas == 1 && isMolecule(m) == isMolecule(n) && isMolecule(m) ~= 0)
                ...
                if reactiveGas == 1 && isMolecule(m) == 0 && isMolecule(n) == 0
                    if 0.5*(m1*(norm(velParallel(1,:)))^2 + m2*(norm(velParallel(2,:)))^2) >= criticalEnergy
                        molCtr = molCtr + 1;
    ...
    if heatTransferBottom == 1
        ...
        velY0 = isBottomWallCollision .* sqrt(velY0.^2 + 2*heatIn*timeStep./massPartArray) + ~isBottomWallCollision.*velY0;
    end

    % add -Y velocity increase due to gravity
    if gravity == 1
        velY0 = velY0 - g*timeStep;
    end
...
for i = 2:itr
    pressure(i) = (massPart(1)*dot(velY(i-1,1:nPart_A),isTopWallCollision(i,1:nPart_A))*2)/timeStep/box_ymax; % (m*dv/dt)/L
    if twoParticleMix == 1
        pressure(i) = pressure(i) + (massPart(2)*dot(velY(i-1,nPart_A+1:end),isTopWallCollision(i,nPart_A+1:end))*2)/timeStep/box_ymax;
    end
end
