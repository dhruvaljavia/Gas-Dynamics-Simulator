...
% Particle A
massPart_A = 1; % mass of particles 
nPart_A = 1000; % no. of particles
size_A = 1; % particle diameter
...
heatIn = 0.5; % in units of energy/time
...
for i = 2:itr
    isTopWallCollision(i,:) = velY(i-1,:)./velY(i,:) == -1 & velY(i,:) < 0;
end
pressure = zeros(itr,1);
for i = 2:itr
    pressure(i) = -(massPart(1)*dot(velY(i,1:nPart_A),isTopWallCollision(i,1:nPart_A))*2)/timeStep/box_ymax; % (m*dv/dt)/L where dt=1 for 1 itr
    if twoParticleMix == 1
        pressure(i) = pressure(i) + -(massPart(2)*dot(velY(i,nPart_A+1:end),isTopWallCollision(i,nPart_A+1:end))*2)/timeStep/box_ymax;
    end
end
