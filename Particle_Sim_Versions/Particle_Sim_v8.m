...
isCollisionPart = zeros(nPart_A+nPart_B,nPart_A+nPart_B); % particle collision detection matrix
...
for i = 2:time
    ...
    % detect inter-particle collision and inside wall collision
    for m = 1:(nPart_A+nPart_B)-1
        for n = m+1:(nPart_A+nPart_B)
            distPart = sqrt((X(i,m) - X(i,n))^2 + (Y(i,m) - Y(i,n))^2);
            isCollisionPart(m,n) = distPart < size((m>nPart_A)+1)/2 + size((n>nPart_A)+1)/2;
        end
    ...
    if innerPartitions == 1
        for w = 1:noOfWalls
            ...
            isCollisionWallEdge(w,1:nPart_A) = inpolygon(X(i,1:nPart_A),Y(i,1:nPart_A),wallBox_A(:,1,w),wallBox_A(:,2,w));
            if twoParticleMix == 1
                isCollisionWallEdge(w,nPart_A+1:end) = inpolygon(X(i,nPart_A+1:end),Y(i,nPart_A+1:end),wallBox_B(:,1,w),wallBox_B(:,2,w));
            end
        end
    end
