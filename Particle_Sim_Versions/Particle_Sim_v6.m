...
grid on;
title('Select rectangle region of initial location of A (and B) particles and press any key');
rect_A = drawrectangle;
if twoParticleMix == 1
    rect_B = drawrectangle;
end
pause;
...
initX_A = 2*(box_xmax/2-size_A)*rand(1,nPart_A) + -(box_xmax/2-size_A); % initial X-coordinates
initY_A = 2*(box_ymax/2-size_A)*rand(1,nPart_A) + -(box_ymax/2-size_A); % initial Y-coordinates
initX_B = 2*(box_xmax/2-size_B)*rand(1,nPart_B) + -(box_xmax/2-size_B);
initY_B = 2*(box_ymax/2-size_B)*rand(1,nPart_B) + -(box_ymax/2-size_B);
