...
% Particle A
massPart_A = 3; % mass of particles 
nPart_A = 50; % no. of particles
size_A = 3; % particle diameter
...
for i = 2:time
    ...
    if innerPartitions == 1
        for w = 1:noOfWalls
            ...
            isCollisionWallEdge(w,1:nPart_A) = (~(isCollisionWallEdge(w,1:nPart_A) & in)) & in; % to avoid staying collided consecutively
            if twoParticleMix == 1
                in = inpolygon(X(i,nPart_A+1:end),Y(i,nPart_A+1:end),wallBox_B(:,1,w),wallBox_B(:,2,w));
                isCollisionWallEdge(w,nPart_A+1:end) = (~(isCollisionWallEdge(w,nPart_A+1:end) & in)) & in;
            end
...
if playAnimationOnly == 1
    ...
    for w = 1:noOfWalls
        line(wallPts(:,1,w), wallPts(:,2,w), 'Color', 'black','LineWidth',1);
        drawpolygon('Position',wallBox_A(:,:,w));
        if twoParticleMix == 1
            drawpolygon('Position',wallBox_B(:,:,w));
        end
    end
    axis equal;
    axis([-box_xmax/2 box_xmax/2 -box_ymax/2 box_ymax/2]);
...
if makeVideo == 1
    ...
    p_B = plot(X(1,nPart_A+1:end),Y(1,nPart_A+1:end),'o','MarkerFaceColor','red', 'MarkerSize', size_B*2.5);
    for w = 1:noOfWalls
        line(wallPts(:,1,w), wallPts(:,2,w), 'Color', 'black');
    end
    axis equal;
    axis([-box_xmax/2 box_xmax/2 -box_ymax/2 box_ymax/2]);
    xticks(-box_xmax/2:10:box_xmax/2);
