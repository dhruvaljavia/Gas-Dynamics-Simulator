...
lookup = configureDictionary("cell","double");
collisionCtr = 0;
...
for i = 2:time
    ...
    % Reset grid and isCollision
    lookup = remove(lookup, keys(lookup));

    % Assign particles to grid
    for p = 1:nPart
        cellX = floor((box_xmax/2 + X(i,p))/cellSize_x); 
        cellY = floor((box_ymax/2 + Y(i,p))/cellSize_y);
        key = {[cellX cellY]};
        lookup(key) = p;
    end
    ...
    for m = 1:nPart
        cellX = floor((box_xmax/2 + X(i,m))/cellSize_x); 
        cellY = floor((box_ymax/2 + Y(i,m))/cellSize_y);
