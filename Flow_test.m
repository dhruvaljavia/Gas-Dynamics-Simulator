...
inlet_velocity = 1; % inlet fluid flow velocity
box_xmax = 100; % max box width
box_ymax = 100; % max box breadth
time = 1000; % equal to iterations

size = 2; % particle diameter
nX = floor((box_xmax/size)/2) + 10;
nY = floor((box_ymax/size)/2);
nPart = nX*nY;
xloc = linspace(-box_xmax/2 + size/2 - (nX-1)*2*size,-box_xmax/2 + size/2,nX);
yloc = linspace(-box_ymax/2 + size/2,box_ymax/2 - size/2,nY);
dY = yloc(2) - yloc(1);
ctrY = 1;
minX = min(xloc)-2*size;
